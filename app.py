...
STRATEGY_PRESETS = {
    "🚀 Absolute Longs": {
        ...
        "filters": [
            {"label": "Absolute Longs Flag", "col": "Absolute_Longs", "op": "==", "val": "YES"},
        ],
    ...
    "🎣 Bottom-Fishing": {
        ...
        "filters": [
            {"label": "Bottom Fishing Flag", "col": "Bottom_Fishing", "op": "==", "val": "YES"},
        ],
...
with st.sidebar:
    ...
    for preset_name, preset in STRATEGY_PRESETS.items():
        ...
        bg_col      = "#f0faf5" if (is_active and "Longs" in preset_name) else ("#fdf0f0" if (is_active and "Fish" in preset_name) else "#ffffff")
...
with tab_detail:
    ...
    if selected:
        ...
        if not row.empty:
            row = row.iloc[0]
            st.markdown(f"### {selected} &nbsp; <span style='font-size:14px;color:#888'>{row.get('Sector','')}</span>", unsafe_allow_html=True)
